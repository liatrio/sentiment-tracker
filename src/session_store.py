    ...
    def update_session(self, session_data: SessionData) -> None:
        ...
        with self._lock:
            if session_data.session_id not in self._sessions:
                raise ValueError(f"Session with ID {session_data.session_id} not found for update.")
...
# Need to import datetime if get_session or update_session modifies last_accessed_at directly
# which they now do.
import datetime
